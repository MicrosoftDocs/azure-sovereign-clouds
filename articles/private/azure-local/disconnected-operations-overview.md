...
- Updates, servicing, and onboarding are performed by using offline or staged workflows.
...
Disconnected operations support hyperconverged Azure Local deployments designed to run fully on premises without reliance on Azure or internet connectivity. Each Azure Local deployment is tied to a single site, like a datacenter. Each Azure Local instance can be deployed with additional resiliency or availability within the datacenter, for example by using rack aware clusters. The disconnected operations control plane can be shared across sites, providing a flexible solution for a multi-site private cloud and ensuring a high level of isolation. When a disconnected operations control plane is shared across multiple sites, you are responsible for providing any required private network connectivity between sites. Cross‑site connectivity is customer‑designed and depends on individual network and security requirements.
